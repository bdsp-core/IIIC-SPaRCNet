...
for i=1:size(lut,1)
    ...
    SEG=[eeg_clean;ekg]; 
    yp=num2str(round(100*lut{i,3}(6))/100);
    if length(yp)==1
        yp=[yp,'.00'];
    else
        yp=[yp,repmat('0',1,4-length(yp))];
    end  
    fcn_EEGpic(SEG,ax{i},['GRDA_',num2str(i),'    ',yp]) 
end
...
for i=1:size(lut,1)
    ...
    eeg=seg(1:19,(20*Fs+1):30*Fs);ekg=seg(20,(20*Fs+1):30*Fs);
    SEG=[fcn_bipolar(eeg);ekg];
    yp=num2str(round(100*lut{i,3}(1))/100);
    if length(yp)==1
        yp=[yp,'.00'];
    else
        yp=[yp,repmat('0',1,4-length(yp))];
    end
    fcn_EEGpic(SEG,ax{4+i},['Other_',num2str(i),'    ',yp])
end
hold(ax{5},'on');
