...
for i=1:size(lut,1)
    ...
    SEG=[eeg_clean;ekg]; 
    yp=num2str(round(100*lut{i,3}(2))/100);
    if length(yp)==1
        yp=[yp,'.00'];
    else
        yp=[yp,repmat('0',1,4-length(yp))];
    end  
    fcn_EEGpic(SEG,ax{i},['Seizure_',num2str(i),'    ',yp]) 
end
...
for i=1:size(lut,1)
    ...
    eeg_clean=fcn_cleaningPipeline(eeg,ekg);
    SEG=[eeg_clean;ekg];
    yp=num2str(round(100*lut{i,3}(3))/100);
    if length(yp)==1
        yp=[yp,'.00'];
    else
        yp=[yp,repmat('0',1,4-length(yp))];
    end
    fcn_EEGpic(SEG,ax{4+i},['LPD_',num2str(i),'    ',yp])
end
hold(ax{5},'on');
